...
export function PracticeSubjectPage() {
  ...
  const { gradeSlug, subjectSlug } = useParams<{ gradeSlug: string; subjectSlug: string }>()
  ...
  useEffect(() => {
    ...
    setLoading(true)
    void Promise.all([fetchPracticeTests(gradeNumber, subjectSlug), fetchPracticeSubjects(gradeNumber)]).then(
      ([testRows, subjectRows]) => {
        setTests(testRows)
  ...
  useSeo({
    ...
    description: t('practice.seo.subjectDescription', { grade: gradeNumber, subject: subjectName }),
    ...
    <MarketingShell>
      <div className="mx-auto max-w-4xl px-4 py-12">
        ...
        {!loading && tests.length === 0 && (
          <Card className="mt-8 text-center text-slate-500">{t('practice.noTestsForSubject')}</Card>
        )}
        ...
        <div className="mt-8 space-y-3">
          {tests.map((test) => {
            ...
            return (
              <Link key={test.id} to={`/practice/grade-${gradeNumber}/${subjectSlug}/${test.slug}`} className="block">
                <Card className="transition-shadow hover:shadow-md">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    <p className="font-bold text-slate-900">{practiceTestTitle(test, language)}</p>
                    <p className="text-xs font-semibold text-slate-400">
                      {t('practice.questionCount', { count: test.questionCount })}
                      {test.languages.length > 1 ? ` · ${t('practice.bothLanguages')}` : ''}
                    </p>
                  </div>
