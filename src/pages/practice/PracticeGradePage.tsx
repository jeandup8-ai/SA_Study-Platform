...
import { fetchPracticeSubjects, type PracticeSubjectSummary } from '@/lib/practice/queries'
  ...
  return (
    <MarketingShell>
      <div className="mx-auto max-w-4xl px-4 py-12">
        <PracticeBreadcrumbs items={[{ label: t('practice.gradeLabel', { grade: gradeNumber }) }]} />
        ...
        {!loading && subjects.length === 0 && (
          <Card className="mt-8 text-center text-slate-500">{t('practice.noTestsForGrade')}</Card>
        )}

        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          {subjects.map((subject) => (
            <Link key={subject.subjectId} to={`/practice/grade-${gradeNumber}/${subject.slug}`}>
