      ...
      <div className="mx-auto max-w-4xl px-4 py-16">
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2">
          {plans.map((plan) => {
            ...
            return (
              <Card key={plan.id} className={isAnnual ? 'relative ring-2 ring-brand-500' : 'relative'}>
        ...
        {plans.length > 0 && (
          <>
            <div className="mt-16">
              <h2 className="text-center text-2xl font-extrabold text-slate-900">{t('pricing.includedTitle')}</h2>
              <ul className="mx-auto mt-6 grid max-w-2xl gap-4 sm:grid-cols-2">
                {CHECKLIST_KEYS.map((key, i) => {
                  const Icon = CHECKLIST_ICONS[i]
                  return (
                    <li key={key} className="flex items-start gap-3 rounded-2xl border border-slate-200 p-4">
                      <Icon size={20} className="mt-0.5 shrink-0 text-brand-600" />
                      <span className="text-sm text-slate-700">{t(`pricing.checklist.${key}`)}</span>
                    </li>
                  )
                })}
              </ul>
            ...
            <div className="mt-16">
              <h2 className="text-center text-2xl font-extrabold text-slate-900">{t('pricing.whyUsTitle')}</h2>
              <div className="mx-auto mt-6 grid max-w-2xl gap-6 sm:grid-cols-2">
                {WHY_US_KEYS.map(({ key, icon: Icon }) => (
                  <div key={key} className="rounded-3xl bg-brand-50 p-6">
                    ...
                    <h3 className="mt-3 font-bold text-slate-900">{t(`pricing.whyUs.${key}Title`)}</h3>
                    <p className="mt-1.5 text-sm text-slate-600">{t(`pricing.whyUs.${key}Body`)}</p>
                  </div>
                ))}
              </div>
            </div>
